...
sudo sed -i "s|external_url 'http://gitlab.example.com' *|external_url 'https://192.168.XXX.XXX'|" /etc/gitlab/gitlab.rb
...
sudo echo -e 'nginx['ssl_certificate'] = "/etc/gitlab/ssl/certificate.pem" ' >> /etc/gitlab/gitlab.rb
sudo echo -e 'nginx['ssl_certificate_key'] = "/etc/gitlab/ssl/key.pem" ' >> /etc/gitlab/gitlab.rb
